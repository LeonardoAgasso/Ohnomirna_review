...
def count_singlepair(pair, G_i, G_hg, G_ppi):
	...
	G_ppi = G_ppi.subgraph(common_tars)

	for edge in G_hg.edges():
		if G_ppi.has_edge(edge[0], edge[1]) or G_ppi.has_edge(edge[1], edge[0]):
			count += 1
...
def nm_parameters_singlepair(pair, G_hg, G_ppi, nm_folder_path, n_rand_nets, true_c):
	...
	for nm_file in glob.glob(nm_folder_path + '/*.edge'):
		...
		cs = count_singlepair(pair, temp_G, G_hg, G_ppi)[0]
...
def process_pair(pair, G_int, G_homo_genes, G_ppi, nm_folder, n_rand_nets):

	true_c, len_tar_1, len_tar_2, len_com_tar  = count_singlepair(pair, G_int, G_homo_genes, G_ppi)

	if type(true_c)!=int:
		mu, sigma, n_maj, z_score, p_value = 'not_in_G', 'not_in_G', 'not_in_G', 'not_in_G', 'not_in_G'
	else:
		mu, sigma, n_maj = nm_parameters_singlepair(pair, G_homo_genes, G_ppi, nm_folder, n_rand_nets, true_c)
		if sigma==0 and true_c>=1:
			...
			p_value = 1 - poisson.cdf(z_score, mu)
...
def main(n_cores):
    nm_folder = sys.argv[5]
    n_rand_nets = int(sys.argv[6])
    ...
    add_edges_nodes(sys.argv[4], G_ppi, G_int)

	# header
    print("miRNA_1\tmiRNA_2\tn_tar_1\tn_tar_2\tn_common_targets\tmu\tsigma\tn_relevant_bifan\tn_maj_true\tZ-score\tpValue (nm size="+str(n_rand_nets)+")")

    pool = multiprocessing.Pool(processes=n_cores)

    results = pool.starmap(process_pair, [(pair, G_int, G_homo_genes, G_ppi, nm_folder, n_rand_nets) for pair in list(G_homo_mirnas.edges())])
