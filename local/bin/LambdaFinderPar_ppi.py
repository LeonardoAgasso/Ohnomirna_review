...
def count_singlemirna(mirna, G_i, G_hg, G_pg):
	...
	# induce the subgraph of G_pg with the targets of the miRNA and count the edges
	G_sub_pg = G_pg.subgraph(targets)

	# count edges in the subgraph that are also in G_hg
	for edge in G_sub_pg.edges():
		if G_hg.has_edge(edge[0], edge[1]):
			count += 1
...
def nm_parameters_singlepair(mirna, G_hg, G_pg, nm_folder_path, n_rand_nets, true_c):
	...
	for nm_file in glob.glob(nm_folder_path + '/*.edge'):
		...
		cs = count_singlemirna(mirna, temp_G, G_hg, G_pg)[0]
		if type(cs)==int:
			if cs>=true_c:
				n_maj_true += 1
		else:
...
def process_pair(mirna, G_int, G_homo_genes, G_ppi_genes, nm_folder, n_rand_nets):

	true_c, len_tar = count_singlemirna(mirna, G_int, G_homo_genes, G_ppi_genes)

	if type(true_c)!=int:
		mu, sigma, n_maj, z_score, p_value = 'not_in_G', 'not_in_G', 'not_in_G', 'not_in_G', 'not_in_G'
	else:
		mu, sigma, n_maj = nm_parameters_singlepair(mirna, G_homo_genes, G_ppi_genes, nm_folder, n_rand_nets, true_c)
...
def main(n_cores):
	...
	G_ppi_genes = nx.Graph()

	add_edges_nodes(sys.argv[3], G_homo_genes, G_int)
	add_edges_nodes(sys.argv[4], G_ppi_genes, G_int)
	...
	results = pool.starmap(process_pair, [(mirna, G_int, G_homo_genes, G_ppi_genes, nm_folder, n_rand_nets) for mirna in mirna_list])
