...
def nm_parameters_singlepair(pair, G_hg, nm_folder_path, n_rand_nets, true_c):
	...
	for nm_file in glob.glob(nm_folder_path + '/*.edge'):
		...
		cs = count_singlepair(pair, temp_G, G_hg)[0]
...
def process_pair(pair, G_int, G_homo_genes, nm_folder, n_rand_nets):

	print("processing pair", pair[0], "-", pair[1], file=sys.stderr)

	true_c, len_tar_1, len_tar_2, len_com_tar  = count_singlepair(pair, G_int, G_homo_genes)

	if type(true_c)!=int:
		mu, sigma, n_maj, z_score, p_value = 'not_in_G', 'not_in_G', 'not_in_G', 'not_in_G', 'not_in_G'
	else:
		mu, sigma, n_maj = nm_parameters_singlepair(pair, G_homo_genes, nm_folder, n_rand_nets, true_c)
		if sigma==0 and true_c>=1:
			...
			# pvalue from the poissonian distribution
...
def main(n_cores):
    ...
    results = pool.starmap(process_pair, [(pair, G_int, G_homo_genes, nm_folder, n_rand_nets) for pair in list(G_homo_mirnas.edges())])
