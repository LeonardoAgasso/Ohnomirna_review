...
def parse_args():	# Parse command line arguments
	...
	parser = OptionParser(usage=format_usage('''
		%prog [OPTIONS] couples.tsv  <fasta.fa >output.tsv
	...
	parser.add_option('-f', '--add-fasta', default=False, action='store_true',
						help='Generate a second output file in a simil-fasta format, \
							where the header contains the names of the two aligned \
							sequences while the "body" contains the alignment score (absolute and normalized) \
							and the alignment itself. This option need the option -a to specify \
							the name of the fasta output (default: %default)')

	parser.add_option('-a', '--fasta-name', default='nd', metavar='STRING',
						help='Specify the name of the output fasta file. Must be used 	\
							with -f option (default: %default)')
...
def check_sequences(seq1, seq2, row):	# Check if the sequences contain characters that are not in the alphabet
	for i in seq1:
		if i not in all_nt:
			raise ValueError("sequence {} (row {}, first sequence) contains the unknown character {}".format(seq1, row, i))
	for i in seq2:
		if i not in all_nt:
			raise ValueError("sequence {} (row {}, second sequence) contains the unknown character {}".format(seq2, row, i))
#____________________________________________________________________________________________________________________
...
def main():
	...
	# Couples from the file stored in a list of couples
	couples = read_couples()


	fasta_bool = options.add_fasta
	
	# Fill the dictionary that links miRNA to sequence
	with open(args[0],"r") as handle:
		for record in SeqIO.parse(handle, "fasta"):
			if record.id == "N.A.":
				continue
			else:
				mirseq[record.id] = record.seq

		# For every couple of miRNAs
		row = 0						# row counter
		for i,j in couples:
			
			check_sequences(mirseq[i], mirseq[j], row)

			# Check if the mirna is in the fasta file
			if i in mirseq and j in mirseq:
				pass
			else:
				# Print N.A. as score for the couple
				print("{}\t{}\t{}".format(i, j, "N.A."), file=sys.stdout)
				continue
...
#	SOURCE:	https://biopython.org/docs/1.75/api/Bio.pairwise2.html	
		...
		####################################################################################################
			...
			print("{}\t{}\t{}".format(i, j, a[0][2]), file=sys.stdout)

			#generate another output file in quasi-FASTA format
			if fasta_bool:
				if options.fasta_name == 'nd':
					print("options -a must be specified", file=sys.stderr)
					sys.exit(1)
				else:
					with open(options.fasta_name, 'a') as fasta_file:
						print_fasta_header(i, j, mirseq[i], mirseq[j], fasta_file)
						print(formatted_alignment, file=fasta_file)
